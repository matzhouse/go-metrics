...
func metricshandler(writer http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		fmt.Fprintf(writer, "Error in Marshalling JSON")
	} else {
		fmt.Fprintf(writer, "%s", o)
	}

}

func data(r metrics.Registry) (output httpmetrics) {
	...
	r.Each(func(name string, i interface{}) {
		...
		case metrics.Counter:
			...
			output.metrics = append(series, &metric{"counter", metric.Count()})
